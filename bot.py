...
def start_command(message):
    bot.send_message(message.chat.id,
                     'Чтобы посмотреть список доступных команд, нажмите /help. Чтобы добавить пост к отслеживанию, пришлите ссылку.')
...
def callback_query(call):
    if call.data == '/add':  # вызов команды /add
        bot.send_message(call.from_user.id,
                         'Введите название поста в кавычках (рекомендуется не длиннее 30 символов). Пожалуйста, не используйте в названии символ ' / '.')
...
def get_text_messages(message):
    ...
    if re.fullmatch(r'^https://t\.me/.+/.+$', message.text):
        ...
        if add_post(input_url):
            ...
            bot.send_message(message.chat.id, "Пост уже отслеживается. Хотите добавить другой пост?",
                             reply_markup=markup_inline)
        else:
        ...
        bot.send_message(message.chat.id,
                         'Вы ввели какой-то текст. К сожалению, я еще не такой умный, чтобы вас понять. Если у вас есть пожелания по функционалу или вопросы по моей работе, пожалуйста, напишите моему разработчику',
                         reply_markup=markup_inline)
...
def list_of_posts(call):
    ...
    for i in range(1, len(all_posts)):
        ...
        if old_comments == current_comments:
            button = types.InlineKeyboardButton(text=f'{all_posts[i][4]} ({all_posts[i][2]})',
                                                callback_data=f'/post/{i}/{all_posts[i][4]}')
        else:
            difference = current_comments - old_comments
            button = types.InlineKeyboardButton(text=f'{all_posts[i][4]} ({all_posts[i][2]} +{difference})',
                                                callback_data=f'/post/{i}/{all_posts[i][4]}')
