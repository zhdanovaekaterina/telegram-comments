...
# Создаем словарь, в который кладем ID поста и кол-во комментариев к нему. Данные заносятся, если кол-во комментариев >0
...
# Выводим сообщение с указанием времени отправки, отправителя, id сообщения и текст
            ...
            try:
                for message in client.iter_messages('frontendlab', reply_to=i, reverse=True):
                    if isinstance(message.sender, telethon.tl.types.User):
                        print(message.date, message.sender.first_name, ':', message.id, ':', message.text)
            except:
                print(i, v)
